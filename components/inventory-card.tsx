...
import { Product, RentalSite } from "@prisma/client";
...
export default function InventoryCard({
  ...
  data: Product & { rentalSite: RentalSite | null };
}) {
  const url = `${data.rentalSite?.subdomain}.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}/${data.slug}`;

  return (
    <div className="relative rounded-lg border border-stone-200 pb-10 shadow-md transition-all hover:shadow-xl dark:border-stone-700 dark:hover:border-white">
      <Link
        href={`/rental-site/${data.rentalSiteId}/inventory/${data.id}`}
